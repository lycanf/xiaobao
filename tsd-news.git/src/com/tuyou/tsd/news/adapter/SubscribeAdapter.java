package com.tuyou.tsd.news.adapter;
...
import com.tuyou.tsd.news.utils.UtilsTools;
				...
				if(isCanClick){
					...
					try {
						if(list.get(position).status==1){
							...
							it.setAction(Contents.REMOVE_NEWS_SUB);
							it.putExtra("index", position);
							context.sendBroadcast(it);
						}else{
							if(isClick){
								Intent it = new Intent();
								it.setAction(Contents.ADD_NEWS_SUB);
								it.putExtra("index", position);
								context.sendBroadcast(it);
							}else{
								Intent it = new Intent();
								it.setAction(Contents.CANNOT_NEWS_ADD);
								context.sendBroadcast(it);
							}
						}
