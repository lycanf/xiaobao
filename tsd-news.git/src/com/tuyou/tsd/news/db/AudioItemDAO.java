...
public class AudioItemDAO extends BaseDAO {

	private static AudioItemDAO instance = null;

	private AudioItemDAO(Context mContext) {
		super(mContext, DBOpenHelper.ITEM_TABLE_NAME);
	}

	public static AudioItemDAO getInstance(Context context) {
		if (instance == null)
			instance = new AudioItemDAO(context);
		return instance;
	}
	 ...
	 */
	public long save(AudioItemEntity entity) {
		long rowId = -1;
	 ...
	 * @return
	 */
	public List<AudioItemEntity> readAll() {
		...
		List<AudioItemEntity> apps = null;
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				apps = new ArrayList<AudioItemEntity>();

				AudioItemEntity app = null;
				do {
					app = new AudioItemEntity();
