...
import com.tuyou.tsd.podcast.adapter.SubscribeAdapter;
import com.tuyou.tsd.podcast.base.MyBaseActivity;
import com.tuyou.tsd.podcast.comm.Contents;
import com.tuyou.tsd.podcast.service.AudioPlayerService;
import com.tuyou.tsd.podcast.service.IAudioPlayerService;
import com.tuyou.tsd.podcast.utils.OrderUtils;
...
public class SubscribeActivity extends MyBaseActivity implements OnClickListener{
	...
	private void initCast() {
		...
		filter.addAction(Contents.ADD_SUB);
		filter.addAction(Contents.REMOVE_SUB);
		filter.addAction(Contents.CANNOT_ADD);
	...
	private void initService() {
		this.startService(new Intent("com.tuyou.tsd.podcast.service.AudioPlayerService"));
		this.bindService( new Intent("com.tuyou.tsd.podcast.service.AudioPlayerService"),this.serviceConnection, BIND_AUTO_CREATE);
	}
	...
	private void showData() {
		try {
			...
			listAudio =  countService.getSubscriptionList("podcast");
	...
	class MyBroadCast extends BroadcastReceiver{
		...
		public void onReceive(Context context, Intent it) {
			if(it.getAction().equals(Contents.ADD_SUB)){
				if(sum < 6){
					sum++;
					showTextNum(String.format(getResources().getString(R.string.sub_sum), sum));
					...
					if(sum>=Contents.ADD_BIG_NUM){
						...
						itCannot.setAction(Contents.ADD_SUB_CANNOT);
			...
			}else if(it.getAction().equals(Contents.REMOVE_SUB)){
				if(sum>0){
					sum--;
					showTextNum(String.format(getResources().getString(R.string.sub_sum), sum));
			...
			}else if(it.getAction().equals(Contents.CANNOT_ADD)){
				adapter.setCanClick(true);
