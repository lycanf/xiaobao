...
public class HeardItemDAO extends BaseDAO {

	private static HeardItemDAO instance = null;

	private HeardItemDAO(Context mContext) {
		super(mContext, DBOpenHelper.HEARD_TABLE_NAME);
	}

	public static HeardItemDAO getInstance(Context context) {
		if (instance == null)
			instance = new HeardItemDAO(context);
		return instance;
	}
	 ...
	 */
	public long save(HeardItemEntity entity) {
		long rowId = -1;
	 ...
	 * @return
	 */
	public List<HeardItemEntity> readAll() {
		...
		List<HeardItemEntity> apps = null;
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				apps = new ArrayList<HeardItemEntity>();

				HeardItemEntity app = null;
				do {
					app = new HeardItemEntity();
