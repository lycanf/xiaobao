...
public class FavouriteRecordDAO extends BaseDAO {

	private static FavouriteRecordDAO instance = null;

	private FavouriteRecordDAO(Context mContext) {
		super(mContext, DBOpenHelper.FAVOURITE_RECORD_TABLE_NAME);
	}

	public static FavouriteRecordDAO getInstance(Context context) {
		if (instance == null)
			instance = new FavouriteRecordDAO(context);
		return instance;
	}
	 ...
	 */
	public long save(FavouriteRecordEntity entity) {
		long rowId = -1;
	 ...
	 * @return
	 */
	public List<FavouriteRecordEntity> readAll() {
		...
		List<FavouriteRecordEntity> apps = null;
		...
		if (cursor != null) {
			if (cursor.moveToFirst()) {
				apps = new ArrayList<FavouriteRecordEntity>();

				FavouriteRecordEntity app = null;
				do {
					app = new FavouriteRecordEntity();
					...
					if (encryptData != null) {
						...
						FavouriteRecord detail = gson.fromJson(gsonString,
								FavouriteRecord.class);
						if (detail != null) {
